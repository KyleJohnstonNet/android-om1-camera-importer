...
class DiagnosticsActivity : ComponentActivity() {
    ...
    private fun vpnStatus(): String {
        ...
        else "VPN is not the main app’s active network. Uploads follow system routing."
    }
    @Composable private fun Screen() {
        ...
        fun browse(path: String, offset: Int = 0) {
            ...
            scope.launch {
                try {
                    val report=CameraHelperClient.list(this@DiagnosticsActivity,path,offset)
        ...
        Scaffold { padding ->
            Column(Modifier.fillMaxSize().padding(padding).verticalScroll(rememberScrollState()).padding(24.dp),verticalArrangement=Arrangement.spacedBy(16.dp)) {
                ...
                Button(enabled=!busy,onClick={
                    try {
                        CameraHelperClient.verify(this@DiagnosticsActivity)
                ...
                Button(enabled=!busy,onClick={
                    ...
                    scope.launch {
                        try {
                            val report=CameraHelperClient.read(this@DiagnosticsActivity)
                    ...
                    files.forEach { file ->
                        OutlinedButton(enabled=!busy,onClick={
                            if(file.directory) browse(file.path)
                                ...
                                scope.launch {
                                    try { status=JpegImport.one(this@DiagnosticsActivity,file.path,file.size) }
                ...
                Text("VPN status is a routing observation. Uploads follow Android’s normal routing and system blocking rules.",style=MaterialTheme.typography.bodySmall)
